...
export type AiArtifactKind = ArtifactKind | "action_list";
...
export const DESIGN_STUDIO_SPEC_V1 = `Design studio quality spec v1:
- Treat the output as a real artifact on a canvas, not text wrapped in a card.
- Start from the user's intent, then choose an artifact shape: website, deck, document, or action list.
- Every artifact needs a clear visual system: hierarchy, spacing scale, type pairing, color roles, and interaction states.
- Prefer one strong object in the center canvas over many small panels.
- Use source context as proof, but do not make the source the main result.
- Include built-item metadata the UI can show: title, surface type, readiness, source trail, version reason, and export state.
- When revising, change the artifact directly and summarize the change. Do not reply with instructions the user has to apply manually.
- If the prompt is thin, infer a tasteful default and ask only for details that would materially change the result.
- Generated websites need responsive behavior, hover/focus states, empty/loading/error states, and production copy.
- Generated decks need varied slide layouts and a clear narrative arc.
- Generated documents need real section structure, callouts, and scan-friendly rhythm.`;
